class FilmList {
  ...
  getData (callback) {
    const request = new XMLHttpRequest(); // Represents an XML request using HTTP.
    ...
    request.onload = function () {
      if (request.status === 200) { // if HTTP request is successfull...
        const jsonString = request.responseText; // put the requested responseText into the jsonString variable
